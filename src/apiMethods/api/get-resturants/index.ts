...
export default async (req: any, res: any) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const apiResponse = await axios.get(`${getBaseUrl()}whatseat/restaurants/`, config)
    ...
    try {
      // Assuming you have an API endpoint to fetch customer addresses
      const apiResponse = await axios.post(`${getBaseUrl()}whatseat/restaurants/`, data, config)

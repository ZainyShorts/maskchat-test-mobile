...
const columnHelper = createColumnHelper<InstagramDataType>()
...
const TelegramListTable = ({ tableData }: { tableData?: TelegramDataType[] }) => {
  ...
  const handleDeleteInstagram = (id: number) => {
    ...
    deletInstaGram(id.toString())
      .then(res => {
        showAlert("error", 'Instagram deleted successfully')
  ...
  const columns = useMemo<ColumnDef<InstagramDataType, any>[]>(
    () => [
      ...
      columnHelper.accessor('ig_appid', {
        header: 'App ID',
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            <div className='flex flex-col'>
              <Typography color='text.primary' className='font-medium'>
                {truncateText(row?.original?.ig_appid, 15)}
              </Typography>
            </div>
          </div>
        )
      ...
      columnHelper.accessor('ig_token', {
        header: 'Access Token',
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            <div className='flex flex-col'>
              <Typography color='text.primary' className='font-medium'>
                {row?.original?.ig_token}
              </Typography>
            </div>
          </div>
        )
      ...
      columnHelper.accessor('ig_account_id', {
        header: 'Account ID',
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            <div className='flex flex-col'>
              <Typography color='text.primary' className='font-medium'>
                {row?.original?.ig_account_id}
              </Typography>
            </div>
          </div>
        )
  ...
  return (
    ...
    <ConfirmationModal
                              ...
                              onConfirm={() => selectedMenuId !== null && handleDeleteInstagram(selectedMenuId)}

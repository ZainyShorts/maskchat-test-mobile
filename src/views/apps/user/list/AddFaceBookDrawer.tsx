...
const AddMessengerDrawer = ({ open, handleClose }: Props) => {
  ...
  const onSubmit = (data: MessengerDataType, e: any) => {
    e.preventDefault()
    // setLoading(true)
    console.log("trigger")
    ...
    FaceBook(data)
      .then((res) => {
        console.log(res, "create FeedMesenger")
        showAlert("success", "Mesenger connected successfully")
        setTimeout(() => {
          handleClose()
      ...
      .catch((error) => {
        if (error?.data && error?.data?.feed_to_gpt) {
          ...
          showAlert("error", "Error in Feed Mesenger")
        }
        console.log(error)
      })
      .finally(() => {
...
<Grid item xs={12} sm={6}>
              <CustomTextField
                ...
                {...register("page_id", { required: "Page ID is required" })}
...
export default AddMessengerDrawer
